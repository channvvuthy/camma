<template>
  <div class="p-5 flex font-khmer_os">
    <div class="w-3/5 pr-5">
      ...
      <div class="text-gray-400 text-14px font-khmer_os">
        {{ defaultVideo.course.teacher.name }}
      </div>
    </div>
    <div class="w-2/5 bg-white h-screen border border-gray-200">
      <div class="overflow-y-scroll pt-5 pb-20" style="height: 91vh">
        <div
          class="relative flex items-center cursor-pointer"
          :class="
            defaultVideo._id === course._id ? 'bg-gray-300 py-2' : ' py-1'
          "
          v-for="(course, key) in downloadDetails"
          :key="key"
        >
          <div
            id="vdActive"
            class="absolute left-0 h-full flex justify-center items-center pl-3"
          ...
          <div class="pr-3 cursor-pointer" @click="askForRemove(course)">
            <CloseIcon :width="22" :height="22" fill="#DC2626"/>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getDefaultVideo(video = "", key = null) {
      ...
      if (
        this.defaultVideo.last_watch &&
        this.defaultVideo.last_watch !== undefined
      ) {
        ...
        if (this.downloadDetails[i]._id === this.defaultVideo._id) {
          if (this.downloadDetails[parseInt(i) + 1] !== undefined) {
            ...
            downloaded = downloaded.filter((item) => {
              if (
                item.course._id ===
                this.downloadDetails[parseInt(i) + 1].course._id
              ) {
    ...
    confirmDelete() {
      ...
      let downloaded = this.downloadDetails.filter(
      ...
      if (oldDownloaded) {
        oldDownloaded = oldDownloaded.filter(
          (item) => item._id !== this.removeVideo._id
        );
    ...
    lastWatchVideo($event) {
      ...
      let downloadDetail = download.filter(
        (item) => item.course._id === this.defaultVideo.course._id
      );
