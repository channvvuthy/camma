  ...
  <div class="bg-white h-screen p-5 font-khmer_os">
    ...
    <div v-else>
      <div class="grid grid-cols-3 gap-4 font-khmer_os text-sm">
        <div class="cursor-pointer shadow rounded-2xl" v-for="(h, key) in help" :key="key" @click="getHelpDetail(h)">
          <img :src="h.thumbnail" alt="" />
          <div class="mt-2 px-5 py-3">{{ h.title }}</div>
        </div>
      </div>
    </div>
    <Detail v-if="showDetail" @dismiss="closeDetail" :videoActive="videoActive" />
  </div>
</template>
