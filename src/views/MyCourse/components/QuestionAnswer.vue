      ...
      >
        ...
        >
          ...
          >
            ...
            <div class="cursor-pointer absolute w-8 h-8 bg-custom rounded-full flex items-center justify-center -right-5 -top-5" @click="closeQuestionAndAnswer">
              <CloseIcon :size="20" fill="#FFF"></CloseIcon>
            </div>
          </div>
          <div
            class="
          ...
          >
            ...
            >
              <div
                class="relative"
                :class="window.width <= '1366' ? ' -top-20' : ' -top-56'"
              >
                <Loading />
              </div>
            </div>
            <div v-else>
              ...
              <div class="flex px-5">
                <div
                  class="h-10 w-10 shadow rounded-full bg-center border border-custom"
                  :style="
                    stProfile
                      ? {
                ...
                <div class="flex pl-3 relative items-center flex-1">
                  ...
                  <div
                    ...
                    @click="chooseImage"
                  >
                    <ImageIcon :size="30" fill="#40b366"/>
                  </div>
                  <div
                    class="transform rotate-45 absolute right-4 cursor-pointer"
                    v-if="text || imgPreview"
                    @click="submit"
                  >
              ...
              <!--Box preview image-->
              <div
                class="h-20 m-5 flex justify-between items-start"
                v-if="imgPreview"
              >
              ...
              <div v-if="forums && forums.length" @scroll="onScroll">
                <ul class="mt-5">
                  <li
                    v-for="(forum, key) in forums"
                    :key="key"
                    class="
                      flex-cols
                      justify-start
                      ...
                      hover:bg-gray-100
                      cursor-pointer
                      border-4 border-b-0 border-l-0 border-r-0 border-gray-200
                    "
                    @click="showAnswerForm(forum)"
                  >
                    <div class="flex items-center">
                      <div
                        class="w-10 h-10 shadow rounded-full bg-center"
                        :style="{
                          backgroundImage: `url(${forum.user.photo})`,
                          backgroundSize: `cover`,
                        }"
                      ></div>
                      <div class="ml-3">
                    ...
                    <div v-if="forum.content.photo" class="ml-14 mt-1">
                      <div class="h-20 flex justify-between items-start">
                        <img
                          :src="forum.content.photo"
                          class="max-h-full rounded w-auto"
                        />
                      </div>
                    </div>
...
export default {
  ...
  data() {
    return {
      stProfile: localStorage.getItem("stProfile")
